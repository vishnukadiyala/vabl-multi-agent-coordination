"""Vectorized VABL training on MPE simple_spread.

Adapts train_vabl_vec.py for the MPE environment family. The only
differences from the Overcooked version:
  1. Environment: SimpleSpreadMPE (3 agents, obs_dim=18, 5 discrete actions,
     horizon=25, cooperative navigation)
  2. Reward: shared negative distance to landmarks (dense, no shaping needed)
  3. obs_dim and n_actions are different (18 and 5 vs 1170 and 6 for Overcooked)

Everything else (architecture, PPO, auxiliary loss, fix-path knobs) is
identical. This is intentional: we want to test whether the gradient-
interference pathology reproduces on a different environment family with
different dynamics, reward structure, and observation space.

Usage:
    python -m marl_research.algorithms.jax.train_vabl_vec_mpe \
        --episodes 100000 --n-envs 64 --seed 0 --aux-lambda 0.05 \
        --save results/mpe/mpe_full_seed0.json
"""
...
def train_vabl_mpe(
    ...
    SimpleSpreadMPE = _import_mpe()
    env = SimpleSpreadMPE(num_agents=n_agents_mpe)
    ...
    print(f"VABL JAX on MPE simple_spread")
    ...
    print(f"  aux_lambda={config.aux_lambda}, aux_anneal_fraction={config.aux_anneal_fraction}")
    ...
    # Flax needs a 'vae' rng stream at init time even when use_vae_belief is
    # False (parameter-shape determination); inert for all MPE runs.
    ...
    def compute_logits_and_aux(params, flat_obs, flat_beliefs, flat_t_oh, flat_t_idx, vae_rng):
        def forward_one(idx, obs_i, belief_i, t_oh_i, t_idx_i):
            ...
            logits, _, aux, _ = agent_net.apply(
                params, obs_i, belief_i, t_oh_i, t_idx_i, jnp.ones(n_teammates),
                rngs={"vae": rng_i})
            return logits, aux
        idx = jnp.arange(flat_obs.shape[0])
        return jax.vmap(forward_one)(idx, flat_obs, flat_beliefs, flat_t_oh, flat_t_idx)
    ...
    def compute_policy_kl(params_new, params_old, flat_obs, flat_beliefs, flat_t_oh, flat_t_idx, vae_rng):
        ...
        Direct measurement of consecutive-policy drift (Sigma_pi), ported
        verbatim from train_vabl_vec.py; both forward passes share one
        vae_rng so the comparison is between parameter sets, not noise
        draws. Comparable to the Overcooked R1 numbers at the per-update
        level; note the rollout batch is n_envs x horizon = 1600 steps here
        vs 25600 on Overcooked AA.
        """
        logits_new, _ = compute_logits_and_aux(
            params_new, flat_obs, flat_beliefs, flat_t_oh, flat_t_idx, vae_rng)
        logits_old, _ = compute_logits_and_aux(
    ...
    for iteration in range(n_iterations):
        ...
        for step in range(horizon):
            ...
            # Shared reward (all agents get the same)
        ...
        # Consecutive-policy KL: drift between iteration starts, measured on
        # this iteration's visited states (same placement as train_vabl_vec.py).
        ...
        for _ in range(config.ppo_epochs):
            ...
            agent_state, a_loss = actor_update(
                ...
                flat_actions, flat_next_t_actions, old_lp_sum_flat, advantages_flat,
                jnp.asarray(aux_lambda_eff), rng_upd)
