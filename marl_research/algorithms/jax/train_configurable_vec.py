"""Vectorized training for ConfigurableAgent on Overcooked.

Architecture sweep: tests which components cause the gradient-interference
pathology by swapping recurrence and attention independently.

Uses jax.lax.scan rollout (same as train_vabl_vec_fast.py) for 3x speedup.

Configurations:
  --recurrence {gru, lstm, none}
  --attention  {cross_attn, self_attn, additive, mean_pool}
  --no-aux-loss / --aux-lambda   (enable/disable aux)
  --critic-aux                   (MAAC-style: aux on critic, not actor)

Usage:
    # LSTM + cross_attn with aux
    python -m marl_research.algorithms.jax.train_configurable_vec \
        --recurrence lstm --attention cross_attn --aux-lambda 0.05 \
        --episodes 25000 --seed 0 --save results/arch_sweep/lstm_cross_aux_s0.json

    # MAAC-style critic aux
    python -m marl_research.algorithms.jax.train_configurable_vec \
        --recurrence none --attention mean_pool --critic-aux --aux-lambda 0.05 \
        --episodes 25000 --seed 0 --save results/arch_sweep/maac_aux_s0.json
"""
...
from marl_research.algorithms.jax.configurable_agent import (
    ConfigurableConfig,
    ConfigurableAgent,
    ConfigurableCritic,
    ConfigurableCriticWithAux,
)
...
def train_configurable(
    config: ConfigurableConfig = None,
    ...
    arch_tag = f"{config.recurrence}+{config.attention}"
    aux_tag = "aux" if config.use_aux_loss else "no_aux"
    if critic_aux:
        aux_tag = "critic_aux"

    print(f"Configurable Agent on Overcooked {layout}")
    print(f"  Architecture: {arch_tag} | Aux: {aux_tag}")
    ...
    def agent_step(agent_params, obs_batch, beliefs_batch, prev_actions_batch, rng):
        def per_env(env_idx, env_obs, env_beliefs, env_prev_acts):
            ...
            def per_agent(i):
                ...
                logits, new_b, _ = agent_net.apply(
                    agent_params, env_obs[i], env_beliefs[i], t_oh, t_idx,
        ...
        return jax.vmap(per_env)(
            jnp.arange(n_envs), obs_batch, beliefs_batch, prev_actions_batch)

    def critic_step(critic_params, states_batch):
        if critic_aux:
            return jax.vmap(
                lambda s: critic_net.apply(critic_params, s)[0])(states_batch)
        else:
            return jax.vmap(
                lambda s: critic_net.apply(critic_params, s))(states_batch)

    # ---- Scan-based rollout ----
    def rollout_body(carry, step_idx):
        rng, obs_batch, beliefs, prev_actions, env_state, states_batch, \
            ep_rewards, a_params, c_params = carry
    ...
    def compute_gae_vec(rewards, values, dones):
        def per_env(rew, val, dn):
            def body(gae, t):
                ...
                next_val = jnp.where(
                    idx + 1 < horizon,
                    val[jnp.minimum(idx + 1, horizon - 1)], 0.0)
    ...
    _use_aux_loss = bool(config.use_aux_loss) and not critic_aux

    # ---- Actor update (aux on actor, standard mode) ----
    @jax.jit
    def actor_update(agent_state, flat_obs, flat_beliefs, flat_t_oh, flat_t_idx,
                     flat_actions, flat_next_t_actions, old_lp_sum, advantages_flat,
                     aux_lambda_eff):
        def loss_fn(params):
            ...
            nlp = jnp.take_along_axis(
                lp, flat_actions[..., None], axis=-1).squeeze(-1).sum(axis=-1)
            ratio = jnp.clip(jnp.exp(nlp - old_lp_sum), 0.0, 5.0)
            s1 = ratio * advantages_flat
            s2 = jnp.clip(ratio, 1 - config.clip_param,
                          1 + config.clip_param) * advantages_flat
            p_loss = -jnp.minimum(s1, s2).mean()
            pr = jax.nn.softmax(logits)
            ent = -(pr * lp).sum(axis=-1).mean(axis=-1)
            e_loss = -ent.mean()

            # Aux loss on actor (standard mode)
            ...
            aux_term = (aux_lambda_eff * aux_loss
                        if _use_aux_loss else jnp.zeros_like(aux_loss))
    ...
    # ---- Critic update (with optional aux on critic) ----
    if critic_aux:
        @jax.jit
        def critic_update(critic_state, flat_states, flat_returns,
                          flat_next_t_actions_critic, aux_lambda_eff):
            def loss_fn(params):
                def forward(s):
                    return critic_net.apply(params, s)
                vals_and_aux = jax.vmap(forward)(flat_states)
                vals = vals_and_aux[0]
                aux_logits = vals_and_aux[1]  # [B, n_teammates, n_actions]
                v_loss = ((vals - flat_returns) ** 2).mean()

                # Aux loss on critic features
                aux_lp = jax.nn.log_softmax(aux_logits, axis=-1)
                aux_taken = jnp.take_along_axis(
                    aux_lp, flat_next_t_actions_critic[..., None],
                    axis=-1).squeeze(-1)
                aux_loss = -aux_taken.mean()
                return v_loss + aux_lambda_eff * aux_loss
        ...
        def critic_update(critic_state, flat_states, flat_returns,
            ...
            def loss_fn(params):
                vals = jax.vmap(
                    lambda s: critic_net.apply(params, s))(flat_states)
    ...
    for iteration in range(n_iterations):
        ...
        obs_batch = jnp.stack(
            [obs_dict_batch[a].reshape(n_envs, -1) for a in agent_names], axis=1)
        ...
        buf_obs = outputs['obs']
        buf_actions = outputs['actions']
        buf_log_probs = outputs['log_probs']
        buf_values = outputs['values']
        buf_beliefs = outputs['beliefs']
        buf_states = outputs['states']
        buf_rewards = outputs['rewards']
        buf_dones = outputs['dones']
        ...
        # Flatten for PPO
        flat_obs = buf_obs.transpose(1, 0, 2, 3).reshape(
            n_envs * horizon * n_agents, obs_dim)
        flat_beliefs = buf_beliefs.transpose(1, 0, 2, 3).reshape(
            n_envs * horizon * n_agents, config.hidden_dim)
        flat_actions = buf_actions.transpose(1, 0, 2).reshape(
            n_envs * horizon, n_agents)
        ...
        flat_states = buf_states.transpose(1, 0, 2).reshape(
            n_envs * horizon, obs_dim * n_agents)

        # For critic-aux: need next teammate actions at the per-iteration level
        # Shape: [n_envs * horizon, n_teammates] — use agent 0's perspective
        flat_next_t_actions_critic = next_t_acts_NHA[:, :, 0, :].reshape(
            n_envs * horizon, n_teammates).astype(jnp.int32)
        ...
        for _ in range(config.ppo_epochs):
            agent_state, a_loss = actor_update(
                ...
                advantages_flat, jnp.asarray(aux_lambda_eff))
            critic_state, c_loss = critic_update(
                critic_state, flat_states, returns_flat,
                flat_next_t_actions_critic, jnp.asarray(aux_lambda_eff))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Architecture sweep with ConfigurableAgent")
    ...
    parser.add_argument("--recurrence", default="gru",
                        choices=["gru", "lstm", "none"],
                        help="Recurrence type")
    parser.add_argument("--attention", default="cross_attn",
                        choices=["cross_attn", "self_attn", "additive", "mean_pool"],
    ...
    # MAAC-style
    parser.add_argument("--critic-aux", action="store_true",
                        help="Put aux loss on critic instead of actor (MAAC-style)")

    args = parser.parse_args()

    base_config = ConfigurableConfig(
