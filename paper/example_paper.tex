...
\begin{abstract}
Effective multi-agent coordination often requires reasoning about the hidden intent of others, yet standard approaches in decentralized POMDPs (Dec-POMDPs) typically rely on egocentric recurrent memory or explicit communication channels. In this work, we propose VABL, a framework for implicit coordination via learned latent belief representations, eliminating the need for communication bandwidth. VABL uses an attention-driven update that aggregates \emph{observable} teammate actions into a compact belief state. To ensure this representation captures coordination-relevant factors, we incorporate a lightweight Theory of Mind objective: an auxiliary loss that trains agents to predict their peers' future actions. Empirical results demonstrate that our method reaches consensus significantly faster than recurrent baselines, bridging the gap between independent learning and explicit communication.
\end{abstract}
...
\begin{itemize}
  ...
  \item We demonstrate empirically that \textbf{attention is the critical component}: removing it drops peak performance by 40\%, while auxiliary prediction provides complementary representation shaping during early training.
  \item We show that VABL achieves 3.3$\times$ higher final reward than MAPPO with 2$\times$ better stability (measured by performance collapse), and 12$\times$ lower variance across seeds, demonstrating more reliable coordination learning.
\end{itemize}
...
Recent work has explored attention architectures to model interactions. Multi-Agent Transformer (MAT) \cite{wen2022mat} uses an encoder-decoder architecture for centralized policy learning, achieving strong results but requiring global observation sequences. UPDeT \cite{hu2021updet} decouples policy learning using transformers to handle variable input sizes but still operates on joint observation spaces. Deep Implicit Coordination Graphs (DICG) \cite{li2020deep} utilize learned coordination structures with attention to infer coordination patterns dynamically. VABL differs from these approaches by using attention strictly over observable teammate actions rather than full observation sharing, maintaining decentralized execution suitable for bandwidth-constrained environments.

\textbf{Explicit communication.}
A large line of work introduces differentiable communication channels learned end-to-end, including RIAL/DIAL \cite{foerster2016dial}, CommNet \cite{sukhbaatar2016commnet}, and gating mechanisms such as IC3Net \cite{singh2018ic3net}, ATOC \cite{jiang2018atoc}, and targeted communication (TarMAC) \cite{das2019tarmac}. In contrast, VABL targets coordination in regimes where explicit communication is unavailable; agents instead infer intent from behavioral evidence.
...
Let $\mathbf{A} = (a_{t-1}^1, \ldots, a_{t-1}^{n-1})$ denote the joint teammate action profile, and let $c_{\mathrm{attn}} = \mathrm{Attn}_\theta(\mathbf{A}; b_{t-1}^i)$ and $c_{\mathrm{mean}} = \frac{1}{n-1}\sum_j \psi_\theta(a_{t-1}^j)$ be the attention and mean-pooling contexts, respectively. Then:
\begin{equation}
I(\mathbf{A}; c_{\mathrm{attn}} \mid b_{t-1}^i) \geq I(\mathbf{A}; c_{\mathrm{mean}})
\end{equation}
with strict inequality when teammates have heterogeneous relevance for coordination.
\end{proposition}

\begin{proof}[Proof sketch]
Mean pooling applies uniform weights $\alpha_j = \frac{1}{n-1}$, while attention learns weights $\alpha_j(b_{t-1}^i)$ that depend on the current belief. By the data processing inequality, any deterministic function of $\mathbf{A}$ satisfies $I(\mathbf{A}; f(\mathbf{A})) \leq H(\mathbf{A})$. The insight here is that attention can learn to \emph{suppress irrelevant teammates}: when teammate $k$ provides no coordination-relevant information (i.e., $I(a_{t-1}^k; s_t) = 0$ given other teammates), attention can set $\alpha_k \approx 0$, while mean pooling cannot. This selective weighting preserves more task-relevant information in the context vector. Formal proof in Appendix~\ref{app:proofs}.
\end{proof}

\begin{proposition}[Context-Dependent Sufficient Statistics]
\label{prop:sufficient_stat}
For a coordination task where the optimal policy $\pi^*(a_t^i \mid \mathbf{A}, o_t^i)$ depends on a subset $S_t \subseteq \{1, \ldots, n-1\}$ of teammates that varies with context, attention can compute a sufficient statistic for $S_t$ while mean pooling cannot.
\end{proposition}

\begin{proof}[Proof sketch]
Let $S_t = S(b_{t-1}^i)$ be the context-dependent relevant subset. A sufficient statistic for decision-making is $T(\mathbf{A}) = \{a_{t-1}^j : j \in S_t\}$. Attention with belief-dependent weights can approximate the indicator function $\alpha_j \approx \mathbf{1}[j \in S_t]$ by learning $g_\theta(b_{t-1}^i, \psi_\theta(a_{t-1}^j))$ to output high scores for relevant teammates. Mean pooling, being context-independent, computes a fixed linear combination that cannot adapt to $S_t$.
\end{proof}

\paragraph{}
Propositions~\ref{prop:info_preservation} and \ref{prop:sufficient_stat} explain why removing attention causes a 40\% performance drop in our experiments: mean pooling dilutes coordination-relevant signals with irrelevant teammate information. In environments like Overcooked where only nearby teammates matter for local coordination decisions, this dilution is particularly harmful.

\begin{proposition}[Belief Stability Under Non-Stationarity]
\label{prop:stability}
Let $b_t^{\mathrm{VABL}}$ and $b_t^{\mathrm{RNN}}$ denote belief states from VABL and a standard RNN agent under non-stationary teammate policies $\{\pi_t^j\}_{j \neq i}$. If teammate policies change at time $t^*$, then:
\begin{equation}
\mathbb{E}\left[\|b_{t^*+k}^{\mathrm{VABL}} - b_{t^*+k}^{*}\|^2\right] \leq \mathbb{E}\left[\|b_{t^*+k}^{\mathrm{RNN}} - b_{t^*+k}^{*}\|^2\right]
\end{equation}
for $k \geq 1$, where $b_t^*$ is the optimal belief for the new policy configuration.
\end{proposition}

\begin{proof}[Proof sketch]
VABL's belief update incorporates \emph{current} teammate actions via attention, providing a direct ``anchor'' to present behavior. When teammate policies change, attention immediately reflects the new action distribution. In contrast, standard RNNs must propagate policy change information through recurrent connections, leading to belief lag. This mechanism explains why VABL exhibits lower collapse (38\%) than RNN-based methods like MAPPO (76\%): attention-anchored beliefs adapt faster to the non-stationarity induced by co-learning. See Appendix~\ref{app:proofs} for the complete proof.
\end{proof}

\paragraph{}
Attention and auxiliary prediction serve complementary roles in VABL. Our ablations (Section~\ref{sec:results}) show that \textbf{attention is the critical component}: removing it drops peak performance by 40\%. The auxiliary objective provides complementary benefits:
\begin{enumerate}
...
then any belief representation $b_t^i$ that is predictive of $a_{t+1}^j$ must encode state-relevant latent factors.
\end{lemma}

\textit{(Formal proofs provided in Appendix.)}
...
We evaluate VABL on two cooperative multi-agent domains with partial observability a \textbf{Simple coordination environment} and \textbf{Overcooked-AI}. for description of these environments see Appendix~\ref{app:env_details}

\subsection{Baselines}
We compare against the following baselines, all using identical observation preprocessing, episode length, and total training steps ($2 \times 10^6$ environment steps):

\textbf{QMIX} \cite{rashid2020monotonic}: Value decomposition with monotonic mixing network uses RNN-based agents with no explicit teammate modeling.
\textbf{MAPPO} \cite{yu2022surprising}: Multi-agent PPO with centralized critic. Strong CTDE baseline without attention or auxiliary objectives.
\textbf{VABL (ours)}: Full model with attention-based belief update and auxiliary prediction.


All methods use the same network capacity (hidden dimension 128), learning rate ($5 \times 10^{-4}$), and PPO hyperparameters (clip $\epsilon = 0.2$,10). Baselines were tuned on a held-out seed and frozen before final evaluation.

\paragraph{Baseline fairness.}
All methods use identical preprocessing, training steps ($2 \times 10^6$), and evaluation protocols. Hyperparameters were tuned via grid search on a held-out seed. Details in Appendix~\ref{app:baseline_fairness}.
...
\begin{table}[tb]
\centering
\caption{Performance comparison. Collapse \% = (Best $-$ Final) / Best $\times$ 100; lower is better (more stable). $^\dagger$ indicates $p < 0.05$ vs. VABL.}
...
\midrule
\multicolumn{4}{c}{\textit{Simple Coordination (3 agents, 500 episodes)}} \\
\midrule
QMIX & $-5.5 \pm 0.04^\dagger$ & $2.4 \pm 1.6$ & -- \\
MAPPO & $88.0 \pm 20.3^\dagger$ & $130.7 \pm 24.7$ & 32.7\% \\
VABL (ours) & $\mathbf{103.1 \pm 1.7}$ & $\mathbf{136.2 \pm 12.6}$ & \textbf{24.3\%} \\
\midrule
\multicolumn{4}{c}{\textit{Overcooked (Asymmetric Advantages, 500 episodes)}} \\
\midrule
...
Table~\ref{tab:main_results} presents our main results. On Simple Coordination, VABL achieves 17\% higher final reward than MAPPO with \textbf{12$\times$ lower variance} ($\pm$1.7 vs $\pm$20.3), demonstrating more reliable learning. On Overcooked, VABL achieves \textbf{3.3$\times$ higher final reward} than MAPPO (28.1 vs 8.6). Critically, MAPPO exhibits severe collapse (76\%), it reaches good performance but cannot sustain it while VABL's collapse is only 38\%. This supports our hypothesis that attention-based belief learning enables more stable coordination. QMIX fails entirely on Simple Coordination and never achieves high performance on Overcooked, consistent with prior findings that value decomposition struggles under partial observability \cite{hu2025adaptability}.

\subsection{Component Ablations}

\begin{table}[tb]
\centering
\caption{Ablation study on Overcooked (Asymmetric Advantages). Results show mean $\pm$ std over 2 seeds, 50 episodes. Attention ablation has strongest impact on peak performance.}
\label{tab:ablation}
\begingroup
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{0.95}
...
\caption{Ablation study on Overcooked. ``Full'' = attention + aux loss; ``No Attn'' = mean pooling instead of attention; ``No Aux'' = $\lambda=0$; ``Baseline'' = neither. \textbf{Left}: Final reward (end of training). \textbf{Right}: Best reward (peak during training). Attention is critical: removing it drops peak by 40\%.}
\label{fig:ablation}
\end{figure}

Table~\ref{tab:ablation} and Figure~\ref{fig:ablation} reveal that \textbf{attention is the critical component}: removing attention drops best performance from 100.5 to 60.0 (\textbf{40\% reduction}), while removing auxiliary loss alone has minimal impact on peak performance. The combined ablation (no attention + no auxiliary) yields the lowest final reward (9.1). Visibility stress tests show that reducing teammate observability to 50\% or 25\% significantly degrades peak performance, validating our hypothesis that observable teammate actions are essential for implicit coordination.
...
In our grid search we found that the sensitivity to auxiliary loss weight $\lambda$. Performance is robust for $\lambda \in [0.01, 0.1]$, with $\lambda = 0.05$ achieving the best results. Higher values ($\lambda \geq 0.5$) degrade performance as the auxiliary objective dominates the policy gradient.
...
VABL's auxiliary prediction accuracy reaches 63\% on Simple Coordination and 81\% on Overcooked (Figure~\ref{fig:aux_accuracy}), confirming that beliefs encode teammate-predictive information as predicted by Lemma 1.
...
Our experiments demonstrate three key findings: VABL achieves \textbf{3.3$\times$ higher final reward} than MAPPO on Overcooked while maintaining \textbf{2$\times$ better stability} (38\% vs 76\% collapse); the attention mechanism is \textbf{critical} - removing it drops peak performance by 40\%; and VABL achieves \textbf{12$\times$ lower variance} than MAPPO on Simple Coordination, indicating more reliable learning.

Ablations reveal that attention over teammate actions is the primary driver of VABL's success, with the auxiliary prediction objective providing complementary representation shaping. With proper stabilization, constant auxiliary loss works well throughout training without requiring annealing.

\paragraph{Limitations and future work.}
Three main limitations suggest directions for future work: \textit{Scalability} - we evaluated with 2--3 agents; scaling to $N > 10$ may require sparse attention. \textit{Observation assumptions} - we assume perfect action observability, extending to noisy or delayed observations is important for deployment. \textit{Cross-play} - we train and test agents together, evaluating coordination with unseen partners would better test generalization. Future work could also explore hybrid architectures that combine implicit and explicit communication.
...
\textbf{Simple Coordination Environment.} A lightweight 3-agent coordination task where agents must learn to take synchronized actions to maximize reward. Agents observe their local state and (when visible) teammates' previous actions. This environment isolates the coordination challenge without confounding factors.

\textbf{Overcooked-AI.} A more complex coordination benchmark based on the popular cooking game \cite{carroll2019utility}. Two agents must cooperate to prepare and deliver dishes in a shared kitchen. We use the \texttt{asymmetric\_advantages} layout, which requires implicit role assignment and spatial coordination.
...
\begin{proposition}[Information Preservation - Restated]
Let $\mathbf{A} = (a_{t-1}^1, \ldots, a_{t-1}^{n-1})$ denote the joint teammate action profile. Let $c_{\mathrm{attn}} = \mathrm{Attn}_\theta(\mathbf{A}; b_{t-1}^i)$ and $c_{\mathrm{mean}} = \frac{1}{n-1}\sum_j \psi_\theta(a_{t-1}^j)$. Then $I(\mathbf{A}; c_{\mathrm{attn}} \mid b_{t-1}^i) \geq I(\mathbf{A}; c_{\mathrm{mean}})$.
\end{proposition}

\begin{proof}
We prove this by showing that attention can represent mean pooling as a special case, plus additional representations that preserve more information.
...
Attention computes $c_{\mathrm{attn}} = \sum_{j=1}^{n-1} \alpha_j(b_{t-1}^i) \psi_\theta(a_{t-1}^j)$ where $\alpha_j \in \Delta^{n-2}$ (the simplex).

Setting $g_\theta(b, \psi(a)) = \text{const}$ for all inputs yields $\alpha_j = \frac{1}{n-1}$, recovering mean pooling. Thus, the set of achievable attention contexts \emph{contains} all mean pooling contexts.

\textbf{Step 2: Information-theoretic argument.}
Consider a coordination scenario with two teammate types: \emph{relevant} ($j \in S$) and \emph{irrelevant} ($j \notin S$), where $|S| < n-1$. Define relevance as: $I(a_{t-1}^j; s_t \mid a_{t-1}^S) = 0$ for $j \notin S$ (irrelevant teammates provide no additional state information).

For mean pooling:
\[
c_{\mathrm{mean}} = \frac{1}{n-1}\left(\sum_{j \in S} \psi(a_{t-1}^j) + \sum_{j \notin S} \psi(a_{t-1}^j)\right)
\]
The irrelevant terms add noise, reducing $I(\mathbf{A}_S; c_{\mathrm{mean}})$ where $\mathbf{A}_S = \{a_{t-1}^j : j \in S\}$.

For attention with learned weights $\alpha_j \approx \mathbf{1}[j \in S] / |S|$:
\[
c_{\mathrm{attn}} \approx \frac{1}{|S|}\sum_{j \in S} \psi(a_{t-1}^j)
\]
This excludes the irrelevant terms, preserving $I(\mathbf{A}_S; c_{\mathrm{attn}}) \geq I(\mathbf{A}_S; c_{\mathrm{mean}})$.

\textbf{Step 3: Strict inequality.}
Equality holds only when: (a) all teammates are equally relevant, or (b) attention weights happen to be uniform. In general coordination tasks with heterogeneous teammate relevance, the inequality is strict.
\end{proof}

\begin{proposition}[Context-Dependent Sufficient Statistics - Restated]
For coordination tasks where the relevant teammate subset $S_t = S(b_{t-1}^i)$ varies with context, attention can compute a sufficient statistic for optimal action selection while mean pooling cannot.
\end{proposition}

\begin{proof}
\textbf{Sufficient statistic definition.}
A statistic $T(\mathbf{A})$ is sufficient for decision-making if $\pi^*(a_t^i \mid \mathbf{A}, o_t^i) = \pi^*(a_t^i \mid T(\mathbf{A}), o_t^i)$---i.e., $T$ captures all decision-relevant information from $\mathbf{A}$.

\textbf{Context-dependent relevance.}
In many coordination tasks, which teammates matter depends on the agent's current situation (encoded in $b_{t-1}^i$). For example:
\begin{itemize}
\item In navigation: nearby teammates are relevant; distant ones are not
\item In role assignment: teammates with complementary roles are relevant
\end{itemize}

Let $S_t = S(b_{t-1}^i) \subseteq \{1, \ldots, n-1\}$ denote the context-dependent relevant subset. The minimal sufficient statistic is $T^*(\mathbf{A}) = \{a_{t-1}^j : j \in S_t\}$.

\textbf{Attention can approximate $T^*$.}
Attention weights are computed as:
\[
\alpha_j = \frac{\exp(g_\theta(b_{t-1}^i, \psi(a_{t-1}^j)))}{\sum_k \exp(g_\theta(b_{t-1}^i, \psi(a_{t-1}^k)))}
\]
With sufficient capacity, $g_\theta$ can learn to output:
\[
g_\theta(b_{t-1}^i, \psi(a_{t-1}^j)) = \begin{cases} C & \text{if } j \in S(b_{t-1}^i) \\ -C & \text{otherwise} \end{cases}
\]
for large $C > 0$. This yields $\alpha_j \approx \mathbf{1}[j \in S_t] / |S_t|$, approximating $T^*$.

\textbf{Mean pooling cannot approximate $T^*$.}
Mean pooling computes $c_{\mathrm{mean}} = \frac{1}{n-1}\sum_j \psi(a_{t-1}^j)$ with no dependence on $b_{t-1}^i$. Since $S_t$ varies with $b_{t-1}^i$, no fixed linear combination can compute a sufficient statistic for all contexts.
\end{proof}

\begin{proposition}[Belief Stability Under Non-Stationarity - Restated]
Under teammate policy changes at time $t^*$, VABL beliefs adapt faster than RNN beliefs to the new optimal belief $b_t^*$.
\end{proposition}
...
\]
The RNN receives no direct signal about teammate policy changes---it must infer them from observation sequences, which requires multiple timesteps.

\emph{VABL belief update:}
\[
b_t^{\mathrm{VABL}} = \text{GRU}(b_{t-1}^{\mathrm{VABL}}, [\phi(o_t^i) \| \underbrace{\mathrm{Attn}(\{a_{t-1}^j\}_{j \neq i})}_{\text{direct action signal}}])
\]
VABL receives \emph{direct} teammate action information via attention. When $\pi^j$ changes, the action distribution $p(a_t^j)$ changes immediately, and this signal propagates directly into the belief update.

\textbf{Adaptation speed.}
Define the belief error $\epsilon_t = \|b_t - b_t^*\|$ where $b_t^*$ is the optimal belief for the current policy configuration.

For RNN: After a policy change, $\epsilon_t$ decreases at rate $O(\gamma^{t-t^*})$ where $\gamma < 1$ is the effective ``forgetting rate'' of the GRU. Information about the policy change must propagate through recurrent connections.

For VABL: The attention context $c_t$ immediately reflects the new action distribution. If attention weights remain reasonable, $\epsilon_t$ decreases at rate $O(\gamma'^{t-t^*})$ with $\gamma' < \gamma$ (faster adaptation) because the belief receives a direct update signal, not just a filtered observation signal.

\textbf{Empirical connection.}
This analysis explains the collapse phenomenon: MAPPO's RNN beliefs drift during co-learning (a form of non-stationarity), while VABL's attention-anchored beliefs remain calibrated to current teammate behavior. The 38\% vs 76\% collapse rates reflect this difference in adaptation speed.
\end{proof}

\subsection{Discussion: Connecting Theory to Experiments}

Our theoretical analysis makes three predictions that align with experimental findings:

\begin{enumerate}
\item \textbf{Attention is critical} (Propositions 1-2): Removing attention forces uniform aggregation, losing the ability to focus on relevant teammates. Predicted effect: significant performance drop. Observed: 40\% reduction in peak performance.

\item \textbf{Auxiliary loss complements attention}: The auxiliary objective provides gradients that shape attention weights toward coordination-relevant teammates. Without this signal, attention may learn task-irrelevant weighting. Predicted effect: auxiliary loss helps, but attention is the primary mechanism. Observed: removing aux loss alone has minimal impact; removing attention alone is devastating.

\item \textbf{VABL resists collapse} (Proposition 3): Attention-anchored beliefs adapt faster to co-learning non-stationarity than pure RNN beliefs. Predicted effect: lower performance collapse. Observed: 38\% collapse (VABL) vs 76\% (MAPPO).
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Baseline fairness: all methods share identical training conditions.}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Env Steps} & \textbf{\#Seeds} & \textbf{Key HP Ranges} & \textbf{Best HPs} \\
\midrule
VABL & $2 \times 10^6$ & 5 & $\lambda \in [0.01, 0.5]$, heads $\in \{2,4,8\}$ & $\lambda=0.05$, heads$=4$ \\
MAPPO & $2 \times 10^6$ & 5 & epochs $\in \{5,10,15\}$, $\gamma \in \{0.95,0.99\}$ & epochs$=10$, $\gamma=0.99$ \\
QMIX & $2 \times 10^6$ & 5 & $\tau \in [0.001, 0.01]$, buffer $\in \{5k,10k\}$ & $\tau=0.005$, buffer$=5k$ \\
IPPO & $2 \times 10^6$ & 5 & epochs $\in \{5,10,15\}$, $\gamma \in \{0.95,0.99\}$ & epochs$=10$, $\gamma=0.99$ \\
\midrule
\multicolumn{5}{l}{\footnotesize \textit{Shared:} hidden$=128$, lr$=5\times10^{-4}$, batch$=32$, clip$=0.2$, value norm$=\checkmark$, orth init$=\checkmark$} \\
\bottomrule
\end{tabular}
\end{table}
