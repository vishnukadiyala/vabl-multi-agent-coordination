...
10M-step reruns of AERIAL~\cite{phan2023attention}, 4 of 5 seeds show
a characteristic 10--20-point late-training drop after peak; one fails
to converge (per-seed data in \S\ref{sec:aerial_observation}). This
residual drop is a distinct, gradient-mediated pathology, not the
short-horizon collapse reported in earlier literature.

\textbf{Isolating the mechanism.} We construct \textbf{VABL}
(Variational Attention-based Belief Learning), a minimal architecture
in the same class with no learned message channel, no hidden-state
sharing, and a cleanly separable auxiliary pathway. In our 7-config
VABL ablation on Overcooked AA at 10M, the attention + constant-aux
cell is the only one that exhibits late-training instability while
every configuration reaches the same Best ceiling. The pathology
reproduces on Cramped Room (Cohen's $d = 5.06$) and on SMAX ($d =
1.26$) but is absent on MPE simple\_spread, consistent with
Proposition~\ref{prop:variance}'s environment-dependent threshold.
Three gradient-targeted interventions ($\lambda$-annealing,
stop-gradient on belief into the auxiliary head, and both combined)
converge to a single recovered regime statistically equivalent to
removing the auxiliary loss. A CIFAR-100 cross-domain test finds the
pathology absent in supervised learning, narrowing it to MARL's
...
Five implications of Eq.~\eqref{eq:variance_bound} align with our
experiments (consistencies, not \emph{a priori} predictions):
...
in parameter-shared DQN from exploration aggregation (magnitude-driven
action-value contraction); our pathology is directional aux-vs-policy
gradient interference within a single agent, suggesting distinct
shared-representation MARL failure pathways with tailored fixes.

\paragraph{Closing the loop on AERIAL.}
The AERIAL signature (\S\ref{sec:aerial_observation}: peak untouched,
10--20-point drop, 4/5 seeds) matches what the directional-interference
mechanism predicts for this architecture class. AERIAL does not admit
a clean aux-on/off ablation, so our VABL study fills that gap. We
conjecture any of our three fix paths would recover AERIAL to the
no-aux regime; verification requires a non-trivial fork.
...
attention-based belief encoders trained with constant-weight auxiliary
losses, first observed in published AERIAL at 10M and isolated in VABL
under controlled ablation. Three gradient-targeted interventions all
sever the auxiliary-to-encoder pathway and converge to the no-aux
regime, identifying it as the necessary conduit. Any of $\lambda \leq
0.01$ or annealing, stop-gradient on the belief into the auxiliary
head, or critic-side auxiliary placement suffices; we additionally find
that the widely reported Overcooked coordination collapse is largely a
