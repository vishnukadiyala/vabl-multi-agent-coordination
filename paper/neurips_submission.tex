...
\textbf{The observed instability.} We construct \textbf{VABL}, a
minimal architecture in this class (\S\ref{sec:background}). In a
$2{\times}2$ ablation on Overcooked AA at 10M env steps, the attention
$+$ aux cell has Final50 reward $\sim$10 points below any alternative
and cross-seed standard deviation $2$--$5\times$ higher, while every
cell reaches comparable peak reward (spread $\sim$2 points;
\S\ref{sec:pathology}). The effect reproduces on Overcooked Cramped
Room and SMAX 3v3 but is absent on MPE simple\_spread and on
supervised CIFAR-100.

\textbf{Isolating the mechanism.} A generic
aux-capacity-consumption story and a directional-interference story
both predict the observed $2{\times}2$ pattern. To distinguish them we
run a target-source test (\S\ref{sec:method_distinguishing}) that
holds aux capacity, architecture, and the aux-to-encoder gradient
pathway fixed and varies only the aux target source: teammate
co-learning targets are pathological
($463.2 \pm 9.6$); frozen-initial-policy targets (stationary but
structured) partially recover ($470.2 \pm 4.5$); uniform random
targets (stationary and unstructured) fully recover the no-aux regime
($473.6 \pm 1.9$, indistinguishable from no aux). The
capacity-consumption hypothesis predicts all three to be pathological
and is ruled out. Target structure \emph{and} non-stationarity are
jointly identified as the pathway-specific driver. Three
gradient-pathway interventions ($\lambda$-annealing, stop-gradient on
the belief, critic-side aux placement) all recover the no-aux regime
on Overcooked; on SMAX they reduce cross-seed variance but a residual
mean gap remains, and halving $\lambda$ (to $0.025$) does not improve
SMAX performance, indicating a non-monotonic $\lambda$ response.
...
AA, Cramped Room, and SMAX 3v3; absent on MPE simple\_spread and
CIFAR-100). \textbf{(2)}~A target-source distinguishing experiment
that rules out a generic aux-capacity-consumption alternative and
identifies structured, non-stationary aux targets as the
pathway-specific driver. \textbf{(3)}~Three gradient-pathway
interventions that recover the no-aux regime on Overcooked, with
honest characterization of a residual SMAX mean gap that is
non-monotonic in $\lambda$. \textbf{(4)}~A reproducible 375{+}-run,
5-seed benchmark with anonymized code. As a secondary finding
(\S\ref{sec:budget}), we observe that much of the reported Overcooked
...
Figure~\ref{fig:gradients} plots the gradient decomposition for Full
VABL on Overcooked AA across 5 seeds. Cosine similarity between the
PPO and auxiliary gradients oscillates across both signs and exhibits
substantial temporal variance within each seed; the magnitude ratio
$\|g_{\mathrm{aux}}\| / \|g_{\pi}\|$ stays well below $0.25$
throughout training. The instability therefore persists despite small
aux-gradient magnitude, consistent with a directional rather than
magnitude-driven mechanism.
...
Multi-seed gradient decomposition (Figure~\ref{fig:gradients}, 5 seeds
of Full VABL with the policy and aux gradients tracked every 25
iterations) confirms the directional-not-magnitude reading: the
late-training (final 50\%) cosine between policy and aux gradients has
temporal standard deviation $0.179 \pm 0.014$ across seeds with mean
$\sim 0$, indicating symmetric directional noise rather than a persistent
conflicting direction, and the magnitude ratio stays bounded below
$0.25$ in every seed throughout training. Under stop-gradient and fully
annealed $\lambda$ the aux gradient no longer reaches the encoder
parameters, so the cosine is zero by construction; we report the Full
signature here and defer the multi-seed gradient-decomposition figure
to Figure~\ref{fig:gradients}.

\subsection{Target-Source Distinguishing Test}
\label{sec:target_source}

The fix paths above are consistent with a directional-interference
mechanism but are also consistent with a generic
aux-capacity-consumption alternative in which any aux head attached to
the shared encoder degrades late-training performance regardless of
what the aux gradient encodes. To distinguish these hypotheses we hold
aux capacity, architecture, and the aux-to-encoder gradient pathway
fixed, and vary only the \emph{source} of the aux target
(\S\ref{sec:method_distinguishing}).

\begin{table}[t]
\centering
\caption{Target-source distinguishing test on Overcooked AA (10M steps, 5 seeds). Aux network and gradient pathway are identical to Full; only the target source varies. Teammate co-learning targets exhibit the pathology; frozen-initial-policy targets partially recover; uniform random targets fully recover the no-aux regime.}
...
Frozen initial-policy (stationary) & yes & no  & $470.22$ & $4.52$ \\
Uniform random                     & no  & no  & $\mathbf{473.58}$ & $\mathbf{1.86}$ \\
\midrule
No aux (reference)                 & --- & --- & $473.45$ & $4.04$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:target_source} shows the result. With frozen-policy
targets the cross-seed std falls from $8.55$ to $4.52$ and the mean
recovers to within $\sim 3$ points of No Aux. With uniform random
targets the std drops to $1.86$ (the tightest of any condition in this
paper) and the mean is statistically indistinguishable from No Aux
(Cohen's $d = -0.04$, CI crossing zero; Appendix~\ref{app:tables}).
The capacity-consumption hypothesis predicts all three conditions to
be pathological because the aux network's parameter count and
gradient pathway are identical; the data rule this out.

\emph{Interpretation.} Structured co-learning targets carry
teammate-policy-specific information that drifts across iterations,
yielding an aux-gradient direction that shifts with teammate updates.
Frozen targets remove the drift but retain structured prediction
content; random targets remove both. The pathway through the encoder
is active in all three, but only the structured-and-drifting
combination degrades Final50 stability. We therefore identify
structured, co-learning-non-stationary aux targets as the
pathway-specific driver rather than the aux pathway itself.

\subsection{Aux-Capacity Scaling Control}
\label{sec:capacity_scaling}

As a complementary control to the target-source test, we vary the
aux-MLP hidden dimension across $\{16, 32, 128\}$ and compare against
the canonical $64$, holding the target (teammate co-learning) and all
other hyperparameters fixed. A capacity-consumption hypothesis
predicts monotonic worsening as hidden dim grows; the
directional-interference story predicts no effect of capacity on
Final50.
...
16      & $462.89$ & $9.15$ & $+0.03$ \\
32      & $468.74$ & $5.49$ & $-0.71$ \\
\textbf{64 (canonical)} & $\mathbf{463.21}$ & $\mathbf{9.55}$ & --- \\
...
distinguishable from $64$. Final50 means lie in
$[462.89, 468.74]$ with cross-seed stds in $[5.49, 9.55]$. An
$8\times$ sweep in aux capacity produces no resolvable effect on the
pathology, reinforcing the target-source test's conclusion: capacity
consumption alone cannot explain the Final50 instability.
...
\subsection{Generalization to Hanabi}
\label{sec:hanabi}

To test whether the pathology reproduces on a canonical turn-based
coordination benchmark beyond Overcooked, we run the same $2 \times 2$
ablation on 2-player Hanabi~\cite{bard2020hanabi} from the JaxMARL
suite with our off-the-shelf hyperparameters (no Hanabi-specific
tuning). The absolute scores are well below published self-play
optima because we do not tune Hanabi-specific hyperparameters; our
claim concerns the \emph{relative} 2$\times$2 pattern.

\begin{table}[t]
\centering
\caption{Hanabi 2-player $2 \times 2$ (JaxMARL, 25K episodes, 5 seeds). No Hanabi-specific tuning; absolute scores are low but the relative pattern is unambiguous. Cohen's $d$ for No Aux vs Full = $+2.81$.}
\label{tab:hanabi}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Attn} & \textbf{Aux} & \textbf{Final50} & \textbf{std} \\
\midrule
Full (attn + aux)        & \checkmark & \checkmark & $0.14$ & $0.26$ \\
No Attention (mean + aux) & --         & \checkmark & $0.26$ & $0.30$ \\
No Aux (attn only)       & \checkmark & --         & $\mathbf{2.81}$ & $1.32$ \\
Neither (mean only)      & --         & --         & $1.84$ & $1.62$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:hanabi} shows the result. The two aux-ON cells
($0.14$ and $0.26$) are an order of magnitude lower than the two
aux-OFF cells ($2.81$ and $1.84$); Cohen's $d$ for No Aux vs Full is
$+2.81$, larger than the $+1.40$ we observe on Overcooked AA. The
qualitative $2 \times 2$ pattern differs from AA in one respect: on
AA the pathology occupies only the attention + aux-ON cell, whereas
on Hanabi both aux-ON cells are pathological. We read this as a
sharpening of the claim: the pathway-specific driver is an aux
gradient carrying structured, co-learning-non-stationary teammate-
prediction content into the shared actor encoder; learned attention
is one surface that amplifies this coupling on AA, but on
information-rich turn-based environments the aux gradient disrupts
policy learning even without attention.

\subsection{Training-Budget Artifact (Secondary Finding)}
\label{sec:budget}

A 6-method benchmark on both Overcooked layouts at 10M steps
(Table~\ref{tab:benchmark} in Appendix~\ref{app:tables}) shows no
method collapses at 10M, unlike earlier reports at lower budgets.
Coordination collapse on AA drops from $\sim$46\% at 500K to $<$3\%
at 10M; MAPPO, TarMAC, and CommNet converge stably. The residual
instability we identify is distinct from and much smaller than the
widely reported ``collapse'', which is largely a training-budget
artifact.
