...
XP-MARL~\cite{xu2024xpmarl} uses auxiliary prioritization as a
non-stationarity tool and BEPAL~\cite{huang2025bepal} adds prediction
heads to CTDE belief encoders. None characterize the aux-to-encoder
\emph{directional} gradient interference we identify.
...
capacity loss~\cite{lyle2022capacity}. Recent benchmarking studies show
that algorithm rankings depend sharply on training budget: Formanek et
al.~\cite{formanek2024mirage} find 75\% of offline MARL SOTA claims
reverse under standardized budgets, and Papadopoulos et
al.~\cite{papadopoulos2025extended} report high cross-seed variance on
Overcooked even at 40M--100M steps. Standardized benchmarking
infrastructure such as BenchMARL~\cite{bettini2024benchmarl} has been
proposed as a community remedy. At 10M steps most reported ``collapse''
is insufficient training; the residual instability we identify is
...
\textbf{SMAX 3v3}~\cite{rutherford2024jaxmarl}: combat coordination with
3 allied agents against 3 heuristic enemies (obs dim 75, horizon 100).
50,000 episodes ($\approx$5M env steps). SMAX is JaxMARL's JAX
reimplementation covering both original SMAC~\cite{samvelyan2019starcraft}
and SMAC v2~\cite{ellis2023smacv2} scenarios; we use a 3v3 configuration
with decentralized heuristic enemies. It tests a fundamentally different
...
belief encoder (policy-corrupting). This aligns with Garcin et
al.~\cite{garcin2025actorcritic}, who show that decoupled actor and
critic representations specialize: the actor extracts action-relevant
features while the critic absorbs value and dynamics information, and
auxiliary-value information that is task-specific can increase the
actor's generalization-harming mutual information. The pathology is
thus specific to decentralized actor architectures with shared encoder
parameters, and it survives independent swaps of recurrence (GRU, LSTM)
and attention (cross, additive), ruling out a tuning artifact of any
single choice.

\paragraph{Concurrent MARL representational failures.}
Contemporaneous work~\cite{weng2026exploration} identifies a related
failure in parameter-shared DQN under exploration: shared
representations lose action-discriminative capacity and action-value
gaps contract (a magnitude signal). Our pathology differs in mechanism:
directional interference between auxiliary and policy gradients within
a single agent, diagnosed via cosine oscillation. The two findings
suggest shared-representation MARL fails via distinct pathways,
exploration-aggregation versus aux-to-encoder gradient interference,
each with tailored fixes (grouped learning, state augmentation vs.
stop-gradient, anneal, critic-side aux).
