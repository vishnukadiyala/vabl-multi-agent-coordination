...
design pattern under co-learning non-stationarity, building on Zhai
et al.'s independent observation that naively co-trained
teammate-action prediction can hurt decentralized policy learning. To
isolate the mechanism we construct VABL as a minimal architecture in
the same class and ablate it across Overcooked, SMAX, and MPE. The
pathology appears only when attention and constant-weight aux
co-occur, is environment-dependent (present on coordination-heavy
Overcooked and SMAX, absent on simpler MPE navigation), and is
driven by a small but directionally noisy auxiliary gradient rather
than magnitude dominance. Three gradient-targeted interventions
(annealing, stop-gradient on the belief, critic-side aux placement)
converge to the no-aux regime on Overcooked; on SMAX they reduce
cross-seed variance but leave a residual mean gap, which we flag as
an open question. A follow-up architecture sweep shows the effect
replicates across two learned-attention variants beyond the main
configuration, mean pooling is immune, and critic-side placement
reverses the sign. We additionally
show that the widely reported ``coordination collapse'' on Overcooked
is largely a training-budget artifact. Code, training logs, and
per-seed results for all 350{+} runs are at
...
reaching strong peak reward during training: severe late-training
degradation for MAPPO~\cite{yu2022surprising} and
AERIAL~\cite{phan2023attention}, with communication-based methods
...
artifact: coordination collapse, defined as
$(\mathrm{Best}{-}\mathrm{Final50})/\mathrm{Best}$, drops from
$\sim$46\% at 500K to $<$3\% at 10M, and MAPPO, TarMAC, and CommNet all
converge stably by 10M (Section~\ref{sec:budget}).
...
comparable Best ($482$--$485$, spread $\sim$2 points). The pathology reproduces on Cramped Room and
...
gradient-interference pathology in the attention + constant-aux
design pattern, isolated under controlled ablation in VABL, with
...
$\mathrm{tr}(\Sigma_\varepsilon)/\lambda_{\min}(H)$, where
$\Sigma_\varepsilon$ is the covariance of the auxiliary-gradient
noise) and gradient decomposition showing the mechanism is directional,
not magnitude-driven; \textbf{(3)}~three gradient-targeted fix paths
...
replicated across two learned-attention variants beyond the main
configuration in a 60-run architecture sweep
(Table~\ref{tab:arch_sweep}); \textbf{(4)}~evidence that the
much-reported Overcooked coordination collapse is largely a
