...
\textbf{The design pattern and its reported instability.}
BEPAL~\cite{huang2025bepal} and Dynamic Belief~\cite{zhai2023dynamic}
...
ablation independently reports that naively co-trained
teammate-action prediction drops success below the no-aux baseline,
attributed to teammate non-stationarity. Our AERIAL port also shows a
late-training drop but AERIAL has no aux, so its origin is distinct
(\S\ref{sec:aerial_observation}).

\textbf{Isolating the mechanism.} We construct \textbf{VABL}
(Variational Attention-based Belief Learning), a minimal architecture
in this design-pattern class with no learned message channel, no
hidden-state sharing, and a cleanly separable auxiliary pathway. In
our 7-config VABL ablation on Overcooked AA at 10M, the attention +
constant-aux cell is the only one that exhibits late-training
instability while every configuration reaches the same Best ceiling.
The pathology reproduces on Cramped Room and on SMAX but is absent on
MPE simple\_spread, consistent with
