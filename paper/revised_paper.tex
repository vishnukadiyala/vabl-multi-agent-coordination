...
\begin{table}[tb]
\centering
\caption{Performance comparison across four environments. Collapse \% = (Best $-$ Final) / Best $\times$ 100; lower is better. $^\dagger$\,$p < 0.05$ vs.\ VABL. 5-agent results use Best reward from the MacBook run (500 episodes, 5 seeds).}
...
\midrule
\multicolumn{4}{c}{\textit{Overcooked -- Cramped Room (500 episodes, 5 seeds)}} \\
\midrule
...
\midrule
\multicolumn{4}{c}{\textit{Simple Coordination (5 agents, 500 episodes, 5 seeds)}} \\
\midrule
MAPPO & $44.9 \pm 9.5^\dagger$ & $84.0 \pm 10.4$ & 46.4\% \\
VABL (ours) & $\mathbf{46.2 \pm 19.3}$ & $\mathbf{95.7 \pm 3.3}$ & \textbf{51.7\%} \\
\bottomrule
\end{tabular}
\endgroup
\end{table}
...
To address concerns about undertrained baselines, we extended MAPPO training to 10M environment steps (25,000 episodes $\times$ horizon 400). At 10M steps, MAPPO exhibits \textbf{100\% performance collapse}: peak reward 503, final reward 0. This confirms that the stability problem is fundamental to MAPPO's architecture, not an artifact of insufficient training budget. The VABL 10M comparison is in progress.
