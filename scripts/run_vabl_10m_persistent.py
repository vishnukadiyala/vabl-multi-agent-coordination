#!/usr/bin/env python
"""
Persistent VABL 10M runner with crash recovery.
Saves results incrementally so progress is never lost.
Mirror of run_mappo_10m_persistent.py but for VABL.

Usage:
    python scripts/run_vabl_10m_persistent.py --device cuda
"""
...
RESULTS_PATH = PROJECT_ROOT / "results" / "vabl_10m_persistent.json"
...
def run_single_seed(config_dict, n_episodes, seed, device, progress_data):
    ...
    algorithm = ALGORITHM_REGISTRY["vabl"](
    ...
    buffer = ReplayBuffer(
        buffer_size=min(cfg.training.buffer_size, 500),
...
def main():
    ...
    print("=" * 72)
    print("VABL 10M Persistent Runner")
